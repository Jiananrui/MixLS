...
advanced_model <- mels(
  ...
  id = "id",
  response = "posmood",
  fixed = ~ alone * genderf,  # Include interaction
  bs = ~ alone * genderf, # Include interaction
  ws = ~ alone * genderf, # Include interaction
  nq = 15,  # More quadrature points
  maxiter = 300, # More iteration steps
  tol = 1e-6,  # Stricter convergence
  stage = 3,
  type = "all", # Compare multiple models (independent, linear, quadratic & interaction)
  ridge1 = 0,
  ridge2 = 0.1,
  ridge3 = 0.5, # Larger starting ridge in stage 3 for quicker convergence
  adaptive = 1
)
```
