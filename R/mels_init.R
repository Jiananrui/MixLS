...
#' @param fixed Formula for fixed effects mean model
#' @param bs Formula for between-subject variance model (or NULL)
#' @param ws Formula for within-subject variance model
#' @param response Name of response variable column
#' @return List of initial parameter values and supporting information
init_1 <- function(data, fixed, 
                   bs = NULL, ws = NULL, response) {
  # Fit OLS for the mean model (beta)
  ols_fit <- lm(as.formula(paste(response, paste(deparse(fixed), collapse = " "), sep = " ")), data = data)
  ...
  if (!is.null(ws)) {
    ...
    ws_fit <- lm(update(ws, log_r2 ~ .), data = ws_data)
    tau <- coef(ws_fit)
  ...
  if (!is.null(bs)) {
    ...
    bs_fit <- lm(update(bs, log_r2 ~ .), data = bs_data)
    alpha <- coef(bs_fit)
  ...
  alpha_names <- if (is.null(bs)) {
    names(alpha)
  } else {
    paste0("alpha_", names(alpha))
  }
  tau_names <- if (is.null(ws)) {
    names(tau)
  } else {
...
#' @param fixed Formula for fixed effects mean model
#' @param id Name of ID variable column
#' @param response Name of response variable column
...
run_em <- function(data, matrices, fixed, id, response,
                   init_values, max_iter = 20) {
  # Extract necessary components
  n_fixed <- ncol(matrices$X)
  X <- matrices$X
  y <- data[[response]]
  id_factor <- as.factor(data[[id]]) # Use a factor for efficient grouping
...
init_1_with_em <- function(data, matrices, fixed, 
                           bs = NULL, ws = NULL,
                           id, response) {
  # Get initial values using OLS method
  init_vals <- init_1(data, fixed, 
                      bs, ws, response)
  ...
  if (!is.null(bs)) {
    ...
    if (has_bs_intercept) {
      ...
      init_vals <- run_em(data, matrices, fixed, id, response, init_vals)
    } else {
...
#' @param ws Formula for within-subject variance model for Stage 2
#' @return Vector of initial tau parameters for Stage 2
#' @importFrom stats lm coef residuals update
#' @export
init_2 <- function(data, response, mean_fit, 
                   tau_intercept, ws) {
...
#' @export
init_3 <- function(data, id, response, fit2, model_type = "linear") {
  ...
  id_summary <- data %>%
    ...
    summarize(
      ...
      v = if (n() > 1) var(.data[[response]]) else 0,
      .groups = 'drop'
    )
