...
#' @param fixed Formula for fixed effects mean model (or NULL for intercept-only)
#' @param bs Formula for between-subject variance model
#' @param ws Formula for within-subject variance model
#' @return A list containing design matrices X, U, and Z
#' @importFrom stats model.matrix
#' @export
setup_matrices <- function(data, fixed, bs, ws) {
  if (!is.null(fixed)) {
  ...
  U <- model.matrix(bs, data = data)
  Z <- model.matrix(ws, data = data)
...
#' @export
prepare_model_data <- function(data, fixed = NULL, bs, ws,
                               id, response, nq) {
  # Setup matrices
  matrices <- setup_matrices(data, fixed, bs, ws)
  
  # Get quadrature points
  gh <- get_gh_points(nq)
  
  # Convert ID to numeric factor
  id_numeric <- as.numeric(as.factor(data[[id]]))
  ...
  id_indices <- split(seq_len(nrow(data)), data[[id]])
...
#' @export
format_params_display <- function(params, param_names, fixed, bs, 
                                  ws, stage = 1, model_type = NULL) {
...
#' @export
create_param_names_table <- function(param_names, fixed, bs, 
                                     ws, stage = 1, model_type = NULL) {
...
#' @return A list containing the newly calculated `eb_theta` and `eb_sd`.
#' @export
...
compute_standardized_residuals <- function(params, model_data, eb_estimates) {
  ...
  eb_theta_long <- eb_estimates$eb_theta[id]
  
  # Compute predicted values and residuals in a single operation
  predicted <- XB + sqrt(BS_var) * eb_theta_long
