...
#' @param bs Formula for between-subject variance
#' @param fixed Formula for fixed effects mean (NULL for intercept only)
#' @param ws Formula for within-subject variance (should always be ~ 1 for stage 1)
...
stage1_fit <- function(data, bs, fixed = NULL, ws,
                       id, response, nq, maxiter, tol, init_ridge = 0, adaptive = 1) {
  # Prepare model data
  model_data <- prepare_model_data(
    data, fixed, bs, ws, id, response, nq
  )

  # Initialize parameters with EM refinement
  init_results <- init_1_with_em(
    data, model_data$matrices, fixed, bs, ws, id, response
  )
  ...
  format_params_display(params, param_names, fixed, bs,
                        ws, stage = 1)
  ...
  for (iter in 1:maxiter) {
    ...
    format_params_display(new_params, param_names, fixed, bs, ws, stage = 1)
    ...
    if (adaptive == 1) {
      ...
      eb_prev_theta <- eb_next$eb_theta
      eb_prev_psd <- eb_next$eb_sd
    }
  ...
  hess_final <- stage12_hessian_wrapper(params, model_data, adaptive, eb_theta = eb_prev_theta, eb_sd = eb_prev_psd)
  ...
  descriptive_names <- create_param_names_table(param_names, fixed, bs,
                                                ws, stage = 1)
...
#' @param bs Formula for between-subject variance
#' @param fixed Formula for fixed effects mean (NULL for intercept only)
#' @param ws Formula for within-subject variance
#' @param id Name of ID variable
#' @param response Name of response variable
#' @param nq Number of quadrature points
#' @param maxiter Maximum iterations
...
#' @importFrom stats pnorm terms
#' @export
stage2_fit <- function(stage1_result, data, bs, fixed, ws,
                       id, response, nq, maxiter, tol, init_ridge = 0.1, adaptive = 1) {
  ...
  if (adaptive == 1) {
    ...
    eb_prev_theta <- stage1_result$eb_estimates$eb_theta
    eb_prev_psd <- stage1_result$eb_estimates$eb_sd
  ...
  for (iter in 1:maxiter) {
    ...
    cat(sprintf("\nIteration %d \n", iter))
    cat("Updated parameters:\n")
    format_params_display(new_params, param_names, fixed, bs, ws, stage = 2)
    cat(sprintf("Log-likelihood: %.6f | Change: %.5f | Ridge: %.3f\n",
                current_loglik, current_loglik - prev_loglik, ridge))
...
#' @param data Data frame containing variables
#' @param fixed Formula for fixed effects mean
#' @param bs Formula for between-subject variance
#' @param ws Formula for within-subject variance
#' @param id Name of ID variable
#' @param response Name of response variable
...
#' @return List of model results
#' @export
stage3_fit <- function(stage2_result, data, fixed, bs, ws,
                       id, response, nq, maxiter, tol,
  ...
  if (adaptive == 1) {
    if (is.null(stage2_result$eb_estimates)) {
      ...
      eb_prev_theta2[, 1] <- stage2_result$eb_estimates$eb_theta
      eb_prev_thetav[, 1] <- stage2_result$eb_estimates$eb_sd^2
  ...
  format_params_display(params, param_names, fixed, bs,
                        ws, stage = 3, model_type = model_type)
  ...
  for (iter in 1:maxiter) {
    ...
    format_params_display(new_params, param_names, fixed, bs,
                          ws, stage = 3, model_type = model_type)
    cat(sprintf("Log-likelihood: %.6f | Change: %.5f | Ridge: %.3f\n",
                current_loglik, current_loglik - prev_loglik, ridge))
